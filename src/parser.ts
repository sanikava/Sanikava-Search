...
export async function requestRaw (url: string): Promise<string> {
  ...
  const text = await response.text(); // Use .text() for more accurate results
...
// for google, bing, etc
export async function parseResultList(url: string, options = {} as ParseResultListOptions): Promise<EngineRequest> {
